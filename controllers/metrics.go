...
func init() {
	...
	metrics.Registry.MustRegister(programRoleRequestDurationHistogram)
}

func newRoleRequestHistogram(clusterNamespace, clusterName string, clusterType libsveltosv1alpha1.ClusterType,
...
func programDuration(elapsed time.Duration, clusterNamespace, clusterName, featureID string,
	...
	if featureID == string(libsveltosv1alpha1.FeatureRoleRequest) {
		programRoleRequestDurationHistogram.Observe(elapsed.Seconds())
		clusterHistogram := newRoleRequestHistogram(clusterNamespace, clusterName, clusterType, logger)
